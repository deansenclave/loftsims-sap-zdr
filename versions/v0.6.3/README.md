# LoftSims Complete ZDR Readiness v0.6.3
...
Selecting **System or product type** in the Flavor YAML builder now independently populates the editable product name, flavor ID, installation/product ID, flow ID and name, purpose, routing, provider, and model/service fields. Selecting a specific corpus source in Knowledge Match can subsequently replace these generic system defaults with more precise source-derived values.
...
Selected sources now generate explicit ZDR feature-activation limitations. Each record contains the affected anchor, factor, activation status, published posture, claim scope, exception or limitation, verification requirement, truth class, and human-review flag. These records are embedded in the generated YAML, assessment JSON, 15-anchor results, recommendation document, and complete project export.
